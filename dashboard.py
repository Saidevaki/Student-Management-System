...
class RMS:
    def __init__(self, root):  # Corrected constructor
        ...
        # ==icons====
        self.log_dash = ImageTk.PhotoImage(file="images/circle.jpg")  # Ensure the path is correct
        
        # ===title===
        title = Label(
            self.root,
            text="Student Result Management System",
            padx=10,  # Corrected from 'padax'
            compound=LEFT,
            image=self.log_dash,
...
if __name__ == "__main__":
    root = Tk()
    obj = RMS(root)  # This will now work
    root.mainloop()
